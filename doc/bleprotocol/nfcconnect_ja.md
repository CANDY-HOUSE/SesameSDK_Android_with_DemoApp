...
### NFC（近距離無線通信）接続は通常、主に2つの側面に関連しています：NFCタグとNFCピア・ツー・ピア接続です。以下に、これらの機能をAndroidデバイスでどのように使用するかについての簡単な説明を示します：
#### NFCタグの読み書き：

- NFCタグのデータを読み取る： AndroidのNFC APIを使用して近くのNFCタグを検出し、タグに保存されているデータを読み取ることができます。NfcAdapter.ReaderCallbackを登録して、タグが検出されたときにデータを処理できます。
-  NFCタグにデータを書き込む： タグのNDEF（NFCデータ交換フォーマット）メッセージを設定することにより、タグにデータを書き込むことができます。通常、NdefMessageを作成し、それをタグに書き込む手順が含まれます。


#### NFCピア・ツー・ピア接続：


- NFC Beam： NFC Beamを使用すると、2つのNFC対応のAndroidデバイス間でNFCを介してデータを送受信できます。NfcAdapterのsetNdefPushMessageメソッドを使用して共有するデータを設定し、デバイスを近づけることでデータ転送を開始できます。

#### NFCイベントの処理：
アクティビティ内では、onNewIntentメソッドをオーバーライドして受信したNFCインテント（Intent）を処理できます。これは、デバイスがNFCタグまたは別のNFCデバイスを検出したときに呼び出されるメソッドです。
### 以下は、AndroidでNFCタグのデータを読み取る方法を示す簡単な例です：
