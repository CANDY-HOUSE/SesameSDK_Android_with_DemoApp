...
internal class CHPalmCapableImpl() :
    ...
    override fun palmChange(ID: String, name: String, result: CHResult<CHEmpty>) {
        val byte1 = byteArrayOf(1, ID.toByte())
        sendCommandSafely(SesameOS3Payload(SesameItemCode.SSM_OS3_PALM_CHANGE.value, byte1 + name.toByteArray()), result) { res ->
            result.invoke(Result.success(CHResultState.CHResultStateBLE(CHEmpty())))
        }
    }

    override fun palmDelete(id: Int, result: CHResult<CHEmpty>) {
        sendCommandSafely(SesameOS3Payload(SesameItemCode.SSM_OS3_PALM_DELETE.value, byteArrayOf(id.toByte())), result) {
            result.invoke(Result.success(CHResultState.CHResultStateBLE(CHEmpty())))
        }
    }
