...
internal class CHFaceCapableImpl() :
    ...
    override fun faceChange(ID: String, name: String, result: CHResult<CHEmpty>) {
        val byte1 = byteArrayOf(1, ID.toByte())
        sendCommandSafely(SesameOS3Payload(SesameItemCode.SSM_OS3_FACE_CHANGE.value, byte1 + name.toByteArray()),result) { res ->
            result.invoke(Result.success(CHResultState.CHResultStateBLE(CHEmpty())))
        }
    }

    override fun faceDelete(ID: Int, result: CHResult<CHEmpty>) {
        sendCommandSafely(SesameOS3Payload(SesameItemCode.SSM_OS3_FACE_DELETE.value, byteArrayOf(ID.toByte())),result) {
            result.invoke(Result.success(CHResultState.CHResultStateBLE(CHEmpty())))
        }
    }
    ...
    override fun faceNameSet(faceNameRequest: CHFaceNameRequest, result: CHResult<String>) {
        L.d("harry", "【faceNameSet】faceNameUUID: ${faceNameRequest.faceNameUUID}; faceID: ${faceNameRequest.faceID}; subUUID: ${faceNameRequest.subUUID}; deviceUUID: ${faceNameRequest.stpDeviceUUID}")
        CHAccountManager.setFaceName(faceNameRequest) { it ->
            it.onSuccess {
                val res = it.data
                L.d("harry", "faceNameSet: $res")
