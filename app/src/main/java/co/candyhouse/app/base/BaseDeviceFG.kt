...
abstract class BaseDeviceFG<T : ViewBinding> : BaseNFG<T>() {
    ...
    fun addDelChangeCfp(id: String, name: String, type: String, isAdd: Boolean) {
        setChangeCfp(type, arrayListOf(ChSubCfp(id, name)), isAdd)
    }

    fun addAllChangeCfp(type: String, list: ArrayList<ChSubCfp>, isAdd: Boolean) {
        setChangeCfp(type, list, isAdd)
    }

    private fun setChangeCfp(type: String, list: ArrayList<ChSubCfp>, isAdd: Boolean) {
        L.d("postCfp", "setChangeCfp=$type")
        if (isUpload) {
            mDeviceModel.ssmLockLiveData.value?.apply {
                val c = CHcfp(this.deviceId.toString(), type, list, isAdd)

                CHAccountManager.postCfp(c) {
                    it.onSuccess {
                        L.d("postCfp", "addAllChangeCfp" + "o")
                    }
                    it.onFailure {
                        L.d("postCfp", "addAllChangeCfp" + "f")
                    }
                }
            }
        }
